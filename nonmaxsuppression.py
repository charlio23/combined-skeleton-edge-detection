...
def nonmax(img, O, r, m):
    ...
    res1 = img[xy0.reshape(-1,2).t().chunk(chunks=2,dim=0)].reshape(elr,h,w)*dxy1[:,:,:,0]*dxy1[:,:,:,1]
    res2 = img[x1y0.reshape(-1,2).t().chunk(chunks=2,dim=0)].reshape(elr,h,w)*dxy0[:,:,:,0]*dxy1[:,:,:,1]
    res3 = img[x0y1.reshape(-1,2).t().chunk(chunks=2,dim=0)].reshape(elr,h,w)*dxy1[:,:,:,0]*dxy0[:,:,:,1]
    res4 = img[xy1.reshape(-1,2).t().chunk(chunks=2,dim=0)].reshape(elr,h,w)*dxy0[:,:,:,0]*dxy0[:,:,:,1]
    e0, _ = (res1 + res2 + res3 + res4).max(0)
